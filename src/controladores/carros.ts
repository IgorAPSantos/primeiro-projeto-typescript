...
export const cadastrarCarros = async (req: Request, res: Response) => {
    ...
    try {
        const carro = await knex<Omit<Carro, 'id'>>('carros').insert({ marca, modelo, ano, cor, valor }).returning('*')
...
export const atualizarCarro = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await knex<Carro>('carros').where({ id: Number(id) }).update({ marca, modelo, ano, cor, valor }).returning('*')
